...
public class BasicShip extends BaseEnt {
    ...
    // FULL BUILD
    public BasicShip(int i, Image im, double hp, double pts, BasicArmor arm,
	    BasicEngine eng, BasicGun gun, double gunPt, double engPt,
	    Circle col) {
	ID = i;
	setImg(im);
	health = hp;
	...
	// draw the engine
	if (weapon != null) {
	    weapon.getImg().drawCentered((float) getWepOffX(),
		    (float) getWepOffY());
	}
	// draw the gun
	if (engine != null) {
	    engine.getInGameImg().drawCentered((float) getEngOffX(),
		    (float) getEngOffY());
	}
	// draw the ship
	getImg().setCenterOfRotation((getImg().getWidth() / 2),
		(getImg().getHeight() / 2));
	...
	// update gun
	double gx = (gunPtLength * Math.cos(Math.toRadians(getImg()
		.getRotation()) + theta))
		+ x;
	double gy = (gunPtLength * Math.sin(Math.toRadians(getImg()
		.getRotation()) + theta))
		+ y;
	...
	// update engine
	gx = (engPtLength * Math.cos(Math.toRadians(getImg().getRotation())
		+ theta))
		+ x;
	gy = (engPtLength * Math.sin(Math.toRadians(getImg().getRotation())
		+ theta))
		+ y;
    ...
    public double length(double x, double y) { // How far this point is away
					       // from the origin. -EMR
